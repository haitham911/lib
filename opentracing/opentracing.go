...
func (o *otWrapper) Call(ctx context.Context, req client.Request, rsp interface{}, opts ...client.CallOption) error {
	...
	if err = o.Client.Call(ctx, req, rsp, opts...); err != nil {
		...
		reqjson := MapReq(req)
		span.SetTag("req", string(reqjson))

		rspJSON, _ := json.MarshalIndent(rsp, "", "  ")

		span.SetTag("rsp", string(rspJSON))
...
func MapReq(req client.Request) []byte {

	mapString := make(map[string]string)

	mapString["Service"] = fmt.Sprintf("%v", req.Service())
	mapString["Codec"] = fmt.Sprintf("%v", req.Codec())
	mapString["ContentType"] = fmt.Sprintf("%v", req.ContentType())
	mapString["Method"] = fmt.Sprintf("%v", req.Method())
	body, _ := json.MarshalIndent(req.Body(), "", "  ")
	mapString["body"] = fmt.Sprintf("%v", string(body))
	jsonString, _ := json.Marshal(mapString)

	return jsonString
}
func MapReqsrv(req server.Request) []byte {
